function [output] = contourmode(x,y,mode,dB,xyrange);
...
for mode_num = 1:size(mode, 3)
    curr_mode = mode(:, :, mode_num);

    if (size(curr_mode) == [length(x)-1,length(y)-1])
    ...
    log_mode = 20*log10(abs(transpose(curr_mode)));
    output = log_mode;

    %{
    c = surf(x, y, real(log_mode));
    shading interp
    %}

    
    % Compute and plot contours
    c = contourc(x,y,20*log10(abs(transpose(curr_mode))),dB);
